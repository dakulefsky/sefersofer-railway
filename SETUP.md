...
3. In the Manus dashboard, add these as environment variables:
   ```
...
5. Click **"Run"**
6. Wait for the migration to complete (you should see a success message)
...
5. Customize the confirmation and password reset emails if desired
...
2. Check that your user is authenticated (logged in)
3. Verify that the RLS policies in the SQL migration were applied correctly
...
2. In Supabase Dashboard → Authentication → Email Templates, verify the sender email is correct
...
2. Letter confusions are recorded in `letter_confusion_pairs`
3. Visual morphologies are recorded in `letter_morphology_variants`
4. The next OCR transcription uses this data to improve accuracy
...
1. **Set up your first job:** Go to http://localhost:3000 and create a new transcription job
2. **Upload a manuscript page:** Use the "New Transcription" page
3. **Review and correct:** Use the PageReview interface to correct OCR results
4. **Monitor analytics:** Check the OCR Analytics dashboard to see your accuracy trends
...
3. Check the project's README.md for development guidelines
